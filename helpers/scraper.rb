...
module Scraper
    class MechScraper
        POSTING = '#serp ul li.posted'.freeze
        LOCATION = '#serp ul li.location'.freeze
        COMPANY = '#serp ul li span.hidden-xs a'.freeze
        ...
        def initialize(search_keywords, location)
            ...
            @posting_dates = parse_results(POSTING)
        ...
        def package
            @output_data = (0..29).map do |index|
                @output_data["job#{index}"] = {
                    job_title: (@job_titles[index]).to_s,
                    job_id: (@job_ids[index]).to_s,
                    company_name: (@company_names[index]).to_s,
                    company_id: (@company_ids[index]).to_s,
                    job_link: (@job_links[index]).to_s,
                    location: (@locations[index]).to_s,
                    posting_date: (@posting_dates[index]).to_s
                }
            end
        end
        ...
        def job_links_parser
            job_links = @search_result.links_with(id: /position\d+/).map(&:href)
        ...
        def parse_results(search_criteria)
            ...
            results_array.map{ |ele| ele = "unknown" if ele.nil? || ele == "" }
      end
